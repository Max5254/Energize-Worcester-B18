...
text(origin,-0.8,'0','FontSize',20,'HorizontalAlignment','center')
...
while leftLabelPos > minstart + 0.1
    ...
    text(leftLabelPos,-0.8,[num2str(leftLabelNumber)],'HorizontalAlignment','center','FontSize',20)
...
while rightLabelPos < maxend-0.1
    ...
    text(rightLabelPos,-0.8,[num2str(rightLabelNumber)],'HorizontalAlignment','center','FontSize',20)
    line([rightLabelPos rightLabelPos],[-0.55,-0.45],'Color','k')
end

text(origin, -1.1, 'Percentage', 'FontSize',20, 'HorizontalAlignment','center')
...
for labelIndex=1:numel(allLabels)
    ...
    text(origin+1.3, total-0.6-0.4*(labelIndex), allLabels{labelIndex},'FontSize',20,'VerticalAlignment','middle')
end

title(question, 'FontSize',30,'FontWeight','bold')
